...
async function verificarChipConectado(chip) {
  try {
    ...
    if (state !== 'open') {
      ...
      await addLog('alerta', `Chip ${chip.instancia} desconectado (${state}). Fila pausada automaticamente.`, { chipId: chip.id, state });
...
async function enfileirarCampanha(campanhaId, bypassWindow = false) {
  ...
  // Puxa a Imagem (se houver) e os delays
  ...
  for (const row of disparos.rows) {
    ...
    // Adiciona a imagem ao Worker
    await disparoQueue.add({ 
      disparoId: row.id, numero: row.numero, mensagem, campanhaId, delayMin, delayMax, mediaUrl: media_url 
    });
  }
...
async function retomar() {
  const chips = await pool.query(`SELECT COUNT(*) FROM chips WHERE status='open'`);
  if (parseInt(chips.rows[0].count) === 0) throw new Error('Nenhum chip conectado. Conecte ao menos um chip antes de retomar.');
...
setInterval(async () => {
  ...
  try {
    ...
    for (const row of res.rows) {
      ...
      try { await enfileirarCampanha(row.id, true); } catch(e) {
