...
async function getSheetsClient() {
  ...
  if (credsJson && credsJson.trim().startsWith('{')) {
    ...
    auth = new google.auth.GoogleAuth({ keyFile: tmpFile, scopes: ['https://www.googleapis.com/auth/spreadsheets.readonly'] });
  } else if (process.env.GOOGLE_SERVICE_ACCOUNT_JSON) {
    auth = new google.auth.GoogleAuth({ keyFile: process.env.GOOGLE_SERVICE_ACCOUNT_JSON, scopes: ['https://www.googleapis.com/auth/spreadsheets.readonly'] });
  } else {
